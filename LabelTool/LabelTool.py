...
##        self.loadDir()
    ...
    def loadDir(self, dbg = False):
        ...
        # self.imageDir = os.path.join(r'./', '%03d' %(self.category))
        print(self.imageDir)
    ...
    def mouseClick(self, event):
        if self.STATE['click'] == 0:
            ...
            self.listbox.itemconfig(len(self.bboxIdList) - 1, fg = COLORS[(len(self.bboxIdList) - 1) % len(COLORS)])
            drawCircle(self.mainPanel,self.STATE['x'],self.STATE['y'], 3, fill = COLORS[(len(self.bboxIdList) - 1) % len(COLORS)])
    ...
    def mouseMove(self, event):
        self.disp.config(text = 'x: %.2f, y: %.2f' %(event.x/DEST_SIZE[0], event.y/DEST_SIZE[1])) # 鼠标移动时显示当前位置的坐标
